...
class ReimburseControllerTest {
    ...
    private RequestPostProcessor userId(String userId) {
        return request -> {
    ...
    class ListReimbursesTests {
        ...
        void listReimburses_Success() throws Exception {
            PageResult<ReimburseVO> pageResult = PageResult.of(1L, 1, 10,
        ...
        void listReimburses_WithFilters() throws Exception {
            PageResult<ReimburseVO> pageResult = PageResult.of(0L, 1, 10, Collections.emptyList());
            when(reimburseService.queryReimburses(eq("user001"), any(ReimburseQueryDTO.class)))
                    .thenReturn(pageResult);
    ...
    class VerifyInvoiceTests {
        ...
        void verifyInvoice_Success() throws Exception {
            when(reimburseService.verifyInvoice(eq("inv001"), eq("user002"), eq(1), eq("核验通过")))
                    .thenReturn(true);
            ...
            verify(reimburseService).verifyInvoice(eq("inv001"), eq("user002"), eq(1), eq("核验通过"));
        }

        @Test
        @DisplayName("发票核验 - 失败")
        void verifyInvoice_Failure() throws Exception {
            when(reimburseService.verifyInvoice(eq("inv001"), eq("user002"), eq(0), eq("发票伪造")))
                    .thenReturn(false);
            ...
            verify(reimburseService).verifyInvoice(eq("inv001"), eq("user002"), eq(0), eq("发票伪造"));
        }
    }
